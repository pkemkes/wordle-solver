...
def main():
    ...
    results = pool.map(play_game,
                       [Game(args.wordfile) for _ in range(args.games)])
...
def play_game(game: Game, verbose="") -> bool:
    ...
    guess_results = []
    while True:
        remaining_words = remove_words_with_result_list(
            remaining_words, guesses, guess_results
        )
...
def remove_words_with_result_list(remaining_words: List[str],
                                  guesses: List[str],
                                  guess_results: List[List[LetterResult]])\
                                  -> List[str]:
...
def remove_words(words: List[str], guess: str,
                 guess_result: List[LetterResult]) -> List[str]:
    return [word for word in words
            if not result_is_worse(check_guess(guess, word), guess_result)
            and word != guess]


def result_is_worse(this: List[LetterResult],
                    other: List[LetterResult]) -> bool:
...
def get_args() -> Namespace:
    ...
    parser.add_argument(
        "-g", "--games", type=int, nargs="?", default=1000,
        help="Number of games to be played."
    )
    parser.add_argument(
        "-p", "--processes", type=int, nargs="?", default=16,
        help="Number of processes to be used for the multiprocessing pool."
    )
    parser.add_argument(
        "-w", "--wordfile", type=str, nargs="?", default="valid_words.json",
