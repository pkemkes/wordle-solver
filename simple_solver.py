from wordle import Game, LetterResult, check_guess, is_won, get_guess_result_str
import random
...
def remove_words_with_result_list(remaining_words: List[str], guesses: List[str], guess_results: List[List[LetterResult]]) -> List[str]:
...
def remove_words(words: List[str], guess: str, guess_result: List[LetterResult]) -> List[str]:
    return [word for word in words if not result_is_worse(check_guess(guess, word), guess_result) and word != guess]
